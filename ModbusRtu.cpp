...
int8_t Modbus::poll() {
  ...
  uint8_t u8current = getRxBufferAvailable();
...
/* _____PROTECTED FUNCTIONS_____________________________________________________ */
...
/**
 * @brief
 * This method is wrapper for port->available()
 *
 * @return port available bytes count
 * @ingroup buffer
 */
int Modbus::getRxBufferAvailable() {
...
int8_t Modbus::process_FC1( uint16_t *regs, uint8_t u8size ) {
  ...
  u8bitsno = 0;
  au8Buffer[ u8BufferSize ] = 0; // clear bits map
